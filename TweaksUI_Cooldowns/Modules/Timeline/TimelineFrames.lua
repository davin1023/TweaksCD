...
--
-- Icons slide from right (full duration) to left (ready) based on remaining time.
-- Position formula: xPos = (remaining / duration) * barWidth
-- ============================================================================
...
local function CalculateIconX(remaining, duration)
    ...
    if remaining > maxDur then
        return barWidth - iconW, true  -- Position at right edge
    end
    
    -- Usable width (bar width minus icon width so icon stays within bounds)
    -- Icons use the FULL bar length - no reserved zones
    local usableWidth = barWidth - iconW
    
    -- Calculate position: 0 remaining = left edge, maxDuration = right edge
    local ratio = remaining / maxDur
    local xPos = ratio * usableWidth
