...
function SpellbookHelper:ProcessDrop()
    ...
    if success then
        ...
        if TUICD.Cooldowns and TUICD.Cooldowns.settingsPanel and TUICD.Cooldowns.settingsPanel.RefreshEntriesList then
            TUICD.Cooldowns.settingsPanel:RefreshEntriesList()
        end
...
function SpellbookHelper:RemoveEntry(index)
    ...
    local customTrackerIcons = TUICD.Cooldowns and TUICD.Cooldowns.customTrackerIcons
    local listIndex = nil
    ...
    if listIndex and TUICD.CooldownHighlights and TUICD.CooldownHighlights.RemoveIconSlot then
        TUICD.CooldownHighlights:RemoveIconSlot("custom", listIndex)
    end
    ...
    if TUICD.Cooldowns and TUICD.Cooldowns.settingsPanel then
        if TUICD.Cooldowns.settingsPanel.RefreshEntriesList then
            TUICD.Cooldowns.settingsPanel:RefreshEntriesList()
        end
        if TUICD.Cooldowns.settingsPanel.RefreshEquipmentList then
            TUICD.Cooldowns.settingsPanel:RefreshEquipmentList()
        end
    end
end
